...
int Multicastlistener::operator ()(u_long interfaceindex)
{
    ...
    //setting timeout to RECVFROMTIMEOUTMILLISEC value
    if(setsockopt(multicastsocket, SOL_SOCKET, SO_RCVTIMEO, (char *) &Constants::RECVFROMTIMEOUTMILLISEC, sizeof(Constants::RECVFROMTIMEOUTMILLISEC)) < 0)
    {
        perror("Error from listener setting timeout");
        instancecount--;
        return -1;
    }
    ...
    while(!stop)
    {
        //free buffer
        messagelength = 0;
        while(messagelength < Constants::RECVBUFFERLENGTH)
        {
        ...
        found = -1;
        if(recvfrom(multicastsocket, recvbuffer, Constants::RECVBUFFERLENGTH, 0, (struct sockaddr*) &saddr, &getsize) < 0)
        {
            perror("Multicastrecv timeout");
            continue;
        }
        ...
        if(strncmp(recvbuffer, Constants::MULTICASTSENDERSTOPMESSAGE.c_str(), Constants::MULTICASTSENDERSTOPMESSAGE.length()) == 0)
        {
            ...
            if(found > -1)
            {
                std::cout << "\n" << inet_ntoa(saddr.sin_addr) <<"  " << recvbuffer + Constants::MULTICASTSENDERSTOPMESSAGE.length() <<  " left the network";
...
void Multicastlistener::stoplistener()
{
    stop = true;
}