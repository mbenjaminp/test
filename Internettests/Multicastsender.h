...
public:
    ...
    int operator()(u_long interfaceindex)
    {
        ...
        while(!stop)
        {
            ...
            if(sendto(multicastsocket, Constants::MULTICASTSENDERMESSAGE.c_str(), Constants::MULTICASTSENDERMESSAGE.length(), 0, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in)) < 0)
            {
                perror("sendto failed");
                instancecount--;
                return -1;
        ...
        if(sendto(multicastsocket, Constants::MULTICASTSENDERSTOPMESSAGE.c_str(), Constants::MULTICASTSENDERSTOPMESSAGE.length(), 0, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in)) < 0)
        {
            perror("sendto failed");
            instancecount--;
            return -1;
